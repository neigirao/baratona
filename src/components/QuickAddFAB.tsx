  ...
  const handleAddDrink = async (typeKey: string) => {
    ...
    if (success) {
      ...
      toast({
        ...
        duration: 4000,
        action: {
          label: language === 'pt' ? 'Desfazer' : 'Undo',
          onClick: () => updateConsumption(currentUser.id, 'drink', -1, currentBarId, typeKey),
        },
      } as any);
    }
      ...
      {isOpen && (
        <div className="flex flex-col gap-2 mb-2 animate-fade-in">
          {DRINK_TYPES.map((type, index) => (
            ...
            >
              <span className={cn("text-2xl transition-transform", animatingType === type.key && "scale-125")}>
                {type.emoji}
              </span>
